...
For an existing source response file, treat it as user-selected. Preserve its structure and macros when copying it to the resolved `_rN` output. Make all changes in that output unless `--overwrite` is present. Ask permission before adding or redefining `\ranswer` when no compatible definition exists.
