...
Use the canonical skill directory as the behavioral source of truth on all three hosts. This directory is the repository's Codex distribution mirror; keep its bundled resources synchronized with the canonical package and restrict Codex-only differences to invocation, collaboration, and presentation metadata. These locations and invocation forms were verified against official documentation on 2026-08-10.
...
Do not add Claude-only frontmatter such as `allowed-tools`, `context`, or argument substitutions to the canonical file. Do not add OpenCode-only metadata to the canonical file. Keep the canonical workflow product-neutral; the Codex mirror may name native collaboration tools only in its delegation section.
...
Use copies or symlinks that resolve to the same canonical behavior. Claude Code discovers the first path, Codex discovers the second, and OpenCode discovers either compatibility path. In this repository's `skills/skills-codex/` distribution mirror, keep copied resources byte-aligned where possible and document the narrow Codex-specific SKILL.md differences.
...
6. Confirm `--overwrite` resolves both outputs to their corresponding source paths; do not use this destructive-mode smoke test on valuable files.
