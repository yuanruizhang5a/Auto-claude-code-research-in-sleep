...
def resolve_paths(paper: Path, review: Path, overwrite: bool) -> dict[str, object]:
    ...
    return {
        ...
        "output_tex": str(source_paper if overwrite else revision_path(source_paper)),
        "review_tex": str(source_review),
        "output_review_tex": str(
            source_review if overwrite else revision_path(source_review)
        ),
        "overwrite": overwrite,
    }
...
def main() -> int:
    ...
    try:
        result = resolve_paths(args.paper, args.review, args.overwrite)
